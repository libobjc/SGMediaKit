//
//  SGPlayerHeader_macOS.h
//  SGPlayer_macOS
//
//  Created by Single on 08/12/2016.
//  Copyright © 2016 single. All rights reserved.
//

#import <Foundation/Foundation.h>

FOUNDATION_EXPORT double SGPlayer_tvOSVersionNumber;     // version number
FOUNDATION_EXPORT const unsigned char SGPlayer_tvOSVersionString[];      // version string

// SGPlatform
#import <SGPlayer_tvOS/SGPLFObject.h>

// SGPlayer
#import <SGPlayer_tvOS/SGPlayer.h>
#import <SGPlayer_tvOS/SGPlayerTrack.h>
#import <SGPlayer_tvOS/SGPlayerAction.h>
#import <SGPlayer_tvOS/SGPlayerDecoder.h>
